...
class MUIDataTable extends React.Component {
  ...
  componentDidUpdate(prevProps) {
    ...
    // // On drop searches or filter changes, change page to 0
    if (this.props.savedQuery &&  this.props.savedQuery.filterList !== prevProps.savedQuery.filterList){
      this.changePage(0);
    }
    // Updates the page number when user enter the list view page
    else if (this.props.savedQuery && this.props.savedQuery.page && this.state.page !== this.props.savedQuery.page){
      this.setState({ page: this.props.savedQuery.page});
    }

    if (this.props.savedQuery && this.props.savedQuery.limit && this.state.rowsPerPage !== this.props.savedQuery.limit){
      this.setState({ rowsPerPage: this.props.savedQuery.limit});
    }
    ...
    newColumns.forEach((column, colIndex) => {
      ...
      if (typeof column === 'object') {
        ...
        columnOptions = {
          ...
          filterName: column.filterName ? column.filterName : column.name,
          ...columnOptions,
          ...options,
        };
      } else {
        columnOptions = { ...columnOptions, name: column, label: column, filterName: column };
      }
  ...
  toggleViewColumn = index => {
    this.setState(
      prevState => {
        ...
        this.props.updateColumnViews(columns);      // To update column display changes in ServerRecordsList
        return {
          columns: columns,
        };
      },
      ...
      () => {
        ...
        if (this.options.onColumnSortChange) {
          this.options.onColumnSortChange(
            ...
            this.state.columns[index].filterName
          );
        }
      },
    );
  ...
  changeRowsPerPage = rows => {
    ...
    // pass new page number back to ServerRecordsList
    this.props.updateTablePage(this.state.page, this.state.page * rows, rows);
  };
  ...
  changePage = page => {
    ...
    // pass new page number back to ServerRecordsList
    this.props.updateTablePage(page, page * this.state.rowsPerPage, this.state.rowsPerPage);
  };
  ...
  searchTextUpdate = text => {
    ...
    // Change page back to 0 when search
    this.changePage(0);
  };
    ...
    if (type === 'head') {
      ...
      this.setState(
        prevState => {
          ...
          let selectedMap, newRows, numSelected;
          ...
          if (isDeselect) {
            ...
            selectedMap = buildMap(newRows);
            if (this.options.serverSide) {
              if (newRows.length == displayData.length) {
                numSelected = this.options.count;
              }
              else {
      ...
      this.setState(
        prevState => {
          ...
          let rowPos = -1;
          let numSelected = prevState.numSelected;
