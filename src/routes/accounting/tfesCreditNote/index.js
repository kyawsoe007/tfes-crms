...
import { tfesCreditNoteNewPage, singleTfesCreditNote } from "Helpers/accountingURL";
...
// Actions
import { getCreditNote, getFilterCreditNoteRequest, deleteCreditNote, getSingleCreditNoteRequest, getSavedCreditNoteQuery, clearDuplicate } from "Ducks/creditnotetfes";
import FileCopyOutlinedIcon from "@material-ui/icons/FileCopyOutlined";

class TfesCreditNoteListView extends Component {
    constructor(props) {
        super(props);
        this.state = {
    ...
    invoicePage() {               
        ...
        this.props.history.push(tfesCreditNoteNewPage);
    }

    edit(id) {
        this.props.clearDuplicate();
        this.props.history.push({ pathname: singleTfesCreditNote(id) })
    }

    duplicate(id) {
        this.props.getSingleCreditNoteRequest(id);
        this.props.history.push({ pathname: singleTfesCreditNote(id) }, { isDuplicate: true })
    }

    handleSingleDelete(id) {
        this.props.deleteCreditNote(id);
    }
    ...
    render() {
        const { data, count, loading } = this.props.CreditNoteFiltered;

        const columns = [
            { name: "id", options: { display: "excluded", filter: false, sort: false } },
            { label: "Customer No", name: "custNo", options: { filter: false } },
            { label: "Customer Name", name: "custName", options: { filter: false } },

            {
                label: "Invoice date", name: "date", options: {
                    filter: false,
        ...
        return (
            <div className="rct-block">
                <ServerRecordsList
                    title="CreditNote List"
                    hasSearch={true}
                    columns={columns}
                    data={data}
                    count={count}
                    otherOptions={options}
                    filterFunc={this.props.getFilterCreditNoteRequest}
                    savedQuery={this.props.SavedQuery}
                    getSavedQuery={this.props.getSavedCreditNoteQuery}
                />
            </div>
        )
    }
...
const mapStateToProps = ({ creditNoteState }) => {
    const { CreditNoteFiltered, SavedQuery } = creditNoteState;
    return { CreditNoteFiltered, SavedQuery };
};
export default connect(mapStateToProps, {
    show,
    getCreditNote,
    getFilterCreditNoteRequest,
    deleteCreditNote,
    getSingleCreditNoteRequest,
    getSavedCreditNoteQuery,
    clearDuplicate
})(TfesCreditNoteListView);
