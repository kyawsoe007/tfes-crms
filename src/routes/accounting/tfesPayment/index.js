...
// Actions
import { getFilterPaymentRequest, deletePayment, getSinglePaymentRequest, clearDuplicate, getSavedPaymentQuery } from "Ducks/paymenttfes";

class TfesPaymentListView extends Component {
    constructor(props) {
        super(props);
        this.state = {
    ...
    paymentPage() {
        ...
        this.props.history.push(tfesPaymentNewPage);
    }

    edit(id) {
        this.props.history.push({ pathname: singleTfesPayment(id) })
    }
    ...
    render() {
        const { data, count, loading } = this.props.PaymentFiltered;

        const columns = [
            { name: "id", options: { display: "excluded", filter: false, sort: false } },
            { label: "Payment No", name: "paymentNo", options: { filter: false } },
            { label: "Customer", name: "custName", options: { sort: false, filter: false } },
            ...
            {
                label: "Payment method", name: "paymentMethod", options: {
                    sort: false,
                    filter: false,
            ...
            <div>
                ...
                <div className="rct-block">
                    <ServerRecordsList
                        ...
                        filterFunc={this.props.getFilterPaymentRequest}
                        savedQuery={this.props.SavedQuery}
                        getSavedQuery={this.props.getSavedPaymentQuery}
...
const mapStateToProps = ({ paymentState }) => {
    const { PaymentFiltered, SavedQuery } = paymentState;
    return { PaymentFiltered, SavedQuery };
};
export default connect(mapStateToProps, {
    show,
    getFilterPaymentRequest,
    deletePayment,
    clearDuplicate,
    getSavedPaymentQuery
