...
const EditPaymentMethod = ({
    ...
    const [paymentMethodName, setPaymentMethodName] = useState(PaymentMethodDetail.name)
    const [paymentMethodAccount,setPaymentMethodAccount]=useState(PaymentMethodDetail.account ? PaymentMethodDetail.account._id: "")
    const [paymentMethodJournal,setPaymentMethodJournal]=useState(PaymentMethodDetail.journal ? PaymentMethodDetail.journal._id : "")
    const [paymentCurrency,setPaymentCurrency]=useState(PaymentMethodDetail.currency? PaymentMethodDetail.currency._id : "")
    const [toggle,setToggle]=useState(false)
    const [target,setTarget]=useState('')
    const [accountName,setAccountName]=useState(PaymentMethodDetail.account.accountName)
    
    const closeAndUpdate = () => {

        const patchBody = {
            id:PaymentMethodDetail.id,
            _id:PaymentMethodDetail.id,
            name: paymentMethodName,
            account:paymentMethodAccount,
            journal:paymentMethodJournal,
            currency: paymentCurrency
        }
        patchPaymentMethod(patchBody)
        handleHide();
    }
        ...
        <div className="container" style={{padding: "1rem"}}>
            ...
            <div className="row">
                <div className="col-12">
                    <h1> Edit Payment Method Details</h1>
                </div>
                <div className="col-12">
