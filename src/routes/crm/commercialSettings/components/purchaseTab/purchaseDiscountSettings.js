...
const PurchaseDiscountSettings = ({ show, getAllDiscounts, discountsAll, postDiscount, postDiscountBody, deleteDiscount, deleteDiscountBody, putDiscount, putDiscountBody }) => {
    ...
    useEffect( () => {
        ...
        //guard to prevent undefined error and filter out to show purchase type only 
        if (discountsAll.data && discountsAll.data.type !== "undefined") {
            const filterArr = discountsAll.data.filter( discount => discount.type === "purchase");
    ...
    return (
        <Fragment>
            ...
            <h2 style={{paddingTop:"15px",paddingLeft:"25px"}}>Purchase Discount Settings</h2>
